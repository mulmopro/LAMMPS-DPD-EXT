...
class FixShardlow : public Fix {
 ...
 protected:
  ...
  class PairDPDfdtExt *pairDPDExt;
  class PairDPDfdtEnergy *pairDPDE;
  double (*v_t0)[3];
  int maxRNG;
  double **MATRIX,**AMI;
  void matrix_inverse(double, double, double, double, double, double, double);
 ...
 private:
  ...
  void ssa_update_dpdext(int, int, int);  // Constant Temperature with par/perp forces
