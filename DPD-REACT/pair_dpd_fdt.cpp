...
void PairDPDfdt::compute(int eflag, int vflag)
{
  ...
  double xtmp,ytmp,ztmp,delx,dely,delz,evdwl,fpair, fpairR, fpairD;
  ...
  if (splitFDT_flag) {
    if (!a0_is_zero) for (ii = 0; ii < inum; ii++) {
      ...
      for (jj = 0; jj < jnum; jj++) {
        ...
        fpairD = -gamma_ij*wd*wd*dot*rinv;
        fpairD *= factor_dpd;
        fpairD *= rinv;
        ...
        fpairR *= rinv;
          ...
          if (evflag) ev_tally_SH(i,j,nlocal,newton_pair,
                               evdwl,0.0,fpair, fpairD, fpairR, delx,dely,delz);
        }
      }
    }
  } else {
