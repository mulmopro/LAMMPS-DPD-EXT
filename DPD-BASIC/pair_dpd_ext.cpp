...
/* ----------------------------------------------------------------------
  ...
  Extension of pair style dpd/ext by: Martin Svoboda (ICPF, UJEP), Martin Lísal (ICPF, UJEP)
------------------------------------------------------------------------- */
...
void PairDPDExt::compute(int eflag, int vflag)
{
  ...
  double rsq,r,rinv,dot,wd,wdc,wdPar,wdPerp,randnum,randnumx,randnumy,randnumz;
  ...
  for (ii = 0; ii < inum; ii++) {
	...
	fpair = a0[itype][jtype]*wdc;
        ...
        fpair += factor_sqrt*sigma[itype][jtype]*wdPar*randnum*dtinvsqrt; // JPL: Check this ... factor_sqrt was added by LAMMPS developers
        ...
        prefactor_s = factor_sqrt * sigmaT[itype][jtype]*wdPerp * dtinvsqrt; //JPL: Check this; factor_sqrt was added by LAMMPS developers
	...
	// // JPL: Nunzia needs to check this; comment this for now
        // fpairx *= factor_dpd;
        // fpairy *= factor_dpd;
        // fpairz *= factor_dpd;
	...
	// JPL: Check this block; was this added by LAMMPS developers; Why doesn't appear in Nunzia's code?
        if (eflag) {
  ...
  if (allocated) {
    ...
    for (i = 1; i <= atom->ntypes; i++)
      for (j = i; j <= atom->ntypes; j++)
        // JPL: Check this ... Nunzia doesn't used cutsq and changes definition of cut
	// if (setflag[i][j]) {
        //   cut[i][j] = cut_global;
...
void PairDPDExt::coeff(int narg, char **arg)
{
  ...
  if (narg == 9) cutD_one = utils::numeric(FLERR,arg[8],false,lmp); // JPL: Shouldn't this be cutD_one?
...
double PairDPDExt::init_one(int i, int j)
{
  ...
  return cutD[i][j]; //JPL: Check this; cut vs cutD
}
  ...
  for (int i = 1; i <= atom->ntypes; i++)
    for (int j = i; j <= atom->ntypes; j++)
      fprintf(fp,"%d %d %g %g %g %g %g %g %g\n",i,j,
        a0[i][j],gamma[i][j],gammaT[i][j],ws[i][j],wsT[i][j],cut[i][j],cutD[i][j]);
}
                       ...
                       double /*factor_coul*/, double factor_dpd, double &fforce)
{
  ...
  phi = 0.5*a0[itype][jtype] *cut[itype][jtype]* wdc*wdc; //JPL: Check this; I think Nunzia is missing a factor of cut[itype][jtype]
